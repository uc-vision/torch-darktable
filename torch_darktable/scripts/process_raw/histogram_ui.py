...
class HistogramDisplay:
  ...
  def setup_display(self, fig, main_area_rect, bayer_image, camera_settings):
    ...
    return HistogramResult(
      display_info=self._get_display_info(bayer_image, camera_settings),
      needs_setup=False
    )
  ...
  def _setup_channel_controls(self, fig, main_area_rect):
    ...
    from .ui import create_radio_buttons
    channel_modes = ['All', 'Red', 'Green', 'Blue']
    self._rb_histogram_channels = create_radio_buttons(
      (0, 0, 1, 1), channel_modes, active_channel, 'horizontal'
    )
    ...
    self._channel_controls_axes.add_collection(self._rb_histogram_channels._buttons)
