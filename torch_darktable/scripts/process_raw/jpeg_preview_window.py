...
from ..util import ImageTransform


class JpegPreviewWindow:
    """Popup window that displays JPEG preview with quality/progressive controls."""
    
    def __init__(self, bayer_image, camera_settings, base_pipeline, display_layer):
        """Initialize JPEG preview window."""
        self.bayer_image = bayer_image
        self.camera_settings = camera_settings
        self.base_pipeline = base_pipeline
        self.display_layer = display_layer  # Reference to get current transform
        ...
        self.fig.canvas.manager.set_window_title('JPEG Preview')
        
        # Main image display area - leave space at bottom for controls
        self.img_ax = self.fig.add_axes([0.05, 0.25, 0.9, 0.7])
        self.img_ax.set_aspect('equal')
        self.img_ax.axis('off')
        
        # Quality slider area
        self.slider_ax = self.fig.add_axes([0.15, 0.12, 0.6, 0.04])
        self.quality_slider = Slider(
            self.slider_ax, 'Quality', 1, 100, 
            valinit=self.jpeg_quality, valfmt='%d'
        )
        
        # Progressive checkbox area  
        self.checkbox_ax = self.fig.add_axes([0.15, 0.05, 0.3, 0.05])
        self.checkbox_ax.set_xticks([])
        self.checkbox_ax.set_yticks([])
        self.checkbox_ax.axis('off')
        
        self.progressive_checkbox = CheckButtons(
            self.checkbox_ax, ['Progressive'], [self.jpeg_progressive]
        )
        
        # Info text area
        self.info_ax = self.fig.add_axes([0.5, 0.05, 0.4, 0.05])
        self.info_ax.set_xticks([])
        self.info_ax.set_yticks([])
        self.info_ax.axis('off')
        self.info_text = self.info_ax.text(0, 0.5, '', fontsize=10, 
    ...
    def update_display(self):
        ...
        # Pull current transform from display layer
        current_transform = self.display_layer._user_transform
        # Process image through pipeline with current transform
        processed = self.base_pipeline.process(self.bayer_image, None, current_transform)
        
        # Apply JPEG compression
        jpeg_image, file_size, psnr = self._apply_jpeg_filter(
            processed, self.jpeg_quality, self.jpeg_progressive
        )
    ...
    def _apply_jpeg_filter(self, rgb_image, quality: int, progressive: bool):
