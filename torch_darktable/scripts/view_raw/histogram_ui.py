...
class HistogramDisplay:
  ...
  def _setup_channel_controls(self, fig, main_area_rect):
    ...
    self._channel_controls_axes = fig.add_axes([channel_left, channel_bottom, channel_width, channel_height])
    self._channel_controls_axes.set_xticks([])
    self._channel_controls_axes.set_yticks([])
    ...
    # Create radio buttons directly in the overlay axes
    from .ui_builder import create_radio_buttons

    channel_modes = ['All', 'Red', 'Green', 'Blue']
    self._rb_histogram_channels = create_radio_buttons((0, 0, 1, 1), channel_modes, active_channel, 'horizontal')

    # Replace the auto-created axes with our overlay
    self._rb_histogram_channels.ax.remove()
    self._rb_histogram_channels.ax = self._channel_controls_axes

    # Add components to overlay axes
    for label in self._rb_histogram_channels.labels:
      self._channel_controls_axes.add_artist(label)
    self._channel_controls_axes.add_collection(self._rb_histogram_channels._buttons)  # type: ignore
  ...
  def cleanup(self):
    """Clean up histogram display."""
    if self._histogram_axes is not None:
      self._histogram_axes.remove()
      self._histogram_axes = None

    if self._channel_controls_axes is not None:
      self._channel_controls_axes.remove()
      self._channel_controls_axes = None
