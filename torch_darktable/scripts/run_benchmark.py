...
def run_benchmark(
  ...
  benchmark(
    'Wiener 32x2',
    partial(wiener32x2.process, noise=0.05),
    rgb_tensor,
  ...
  benchmark(
    'Wiener 32x4',
    partial(wiener32x4.process, noise=0.05),
    rgb_tensor,
    warmup_iters=warmup_iters,
    bench_iters=bench_iters,
  )
  benchmark(
    'Wiener 32x2 Gray',
    partial(wiener32x2.process_luminance, noise=0.05),
    rgb_tensor,
    warmup_iters=warmup_iters,
    bench_iters=bench_iters,
  )
  ...
  benchmark(
    'Bilateral 2x2',
    partial(bilateral_2x2.process, detail=0.2),
    mono_tensor,
    warmup_iters=warmup_iters,
    bench_iters=bench_iters,
  )
  benchmark(
    'Bilateral 8x1',
    partial(bilateral_8x1.process, detail=0.2),
    mono_tensor,
    warmup_iters=warmup_iters,
    bench_iters=bench_iters,
  )
...
def main():
  ...
  parser.add_argument(
    ...
    type=str,
    default='RGGB',
    choices=[p.name for p in BayerPattern],
