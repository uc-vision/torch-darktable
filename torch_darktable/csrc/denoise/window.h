...
__constant__ half_t window_interp_16[16];

// 32x32 separable Gaussian windows - half precision for memory efficiency
__constant__ half_t window_fft_32[32];
__constant__ half_t window_interp_32[32];
...
inline void init_windows(const void* fft_symbol, const void* interp_symbol, 
    ...
    // Convert to half precision and copy to provided constant memory symbols
    Half<K> fft_half, interp_half;
    for (int i = 0; i < K; ++i) {
        fft_half[i] = static_cast<half_t>(fft_vals[i]);
        interp_half[i] = static_cast<half_t>(interp_vals[i]);
    }
    
    auto err1 = cudaMemcpyToSymbol(fft_symbol, fft_half.data(), K * sizeof(half_t));
    auto err2 = cudaMemcpyToSymbol(interp_symbol, interp_half.data(), K * sizeof(half_t));
...
struct Window<16> {
    ...
    __device__ __forceinline__ static half_t fft_window(int2 pos) {
        return window_fft_16[pos.x] * window_fft_16[pos.y];
    }
    
    __device__ __forceinline__ static half_t interp_window(int2 pos) {
